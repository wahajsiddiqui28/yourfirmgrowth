...
define( 'YFG_VERSION', '1.8.0' );
define( 'YFG_DIR', get_template_directory() );
define( 'YFG_URI', get_template_directory_uri() );

/**
 * Global contact details — number/link sirf YAHAN change karein,
 * poori site (header, buttons, waghera) mein khud update ho jayega.
 */
define( 'YFG_PHONE_DISPLAY', '+1 (646) 916-5108' );
define( 'YFG_PHONE_E164', '+16469165108' );
define( 'YFG_WHATSAPP_URL', 'https://wa.me/16469165108' );

/**
 * Form notifications kahan jayengi.
 * wp-config.php mein define karke override kiya ja sakta hai.
 * YFG_LEAD_CC mein multiple emails comma se de sakte hain.
 */
if ( ! defined( 'YFG_LEAD_TO' ) ) {
	define( 'YFG_LEAD_TO', 'info@yourfirmgrowth.com' );
}
if ( ! defined( 'YFG_LEAD_CC' ) ) {
	define( 'YFG_LEAD_CC', 'wahajsiddiqui2828@gmail.com' );
}
...
/**
 * Service pages ab header ke neeche wale sub-header mein dikhte hain,
 * is liye unhe primary (top) menu se hata dein — WP admin menu intact rehta hai.
 */
function yfg_remove_services_from_primary_menu( $items, $args ) {
	...
	$service_urls = array_map(
		'untrailingslashit',
		wp_list_pluck( yfg_service_nav_items(), 'url' )
	);

	foreach ( $items as $key => $item ) {
		if ( in_array( untrailingslashit( $item->url ), $service_urls, true ) ) {
			unset( $items[ $key ] );
		}
	}
