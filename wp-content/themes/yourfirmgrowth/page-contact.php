				...
				<div class="pe-lg-4">
					...
					<div class="yfg-contact-info-item">
						...
						<div>
							...
							<p class="yfg-contact-info-item__text"><a href="https://wa.me/16469165108">+1 (646) 916-5108</a> (US Support)</p>
						</div>
					</div>
			...
			<div class="col-lg-7" id="contact-form">
				<div class="card yfg-contact-card p-4 p-md-5">
					...
					<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
						...
						<div class="yfg-field">
							<label>Service of Interest <small class="text-muted fw-normal">(select all that apply)</small></label>
							<div class="yfg-msel" id="yfgServiceMsel">
								<button type="button" class="yfg-msel__toggle" aria-haspopup="true" aria-expanded="false">
									<i class="bi bi-briefcase-fill yfg-msel__icon"></i>
									<span class="yfg-msel__label">Select a service...</span>
									<i class="bi bi-chevron-down yfg-msel__caret"></i>
								</button>
								<div class="yfg-msel__menu">
									<?php
									$yfg_contact_services = array(
										'SEO'                      => 'Search Engine Optimization (SEO)',
										'Web Design & Development' => 'Web Design &amp; Development',
										'Content Marketing'        => 'Content Marketing',
										'Paid Advertising'         => 'Paid Advertising (PPC &amp; Ads)',
										'Branding & Identity'      => 'Branding &amp; Identity',
										'CRO'                      => 'Conversion Rate Optimization (CRO)',
										'Automation'               => 'Business Automation',
										'Consulting'               => 'Digital Growth Consulting',
									);
									foreach ( $yfg_contact_services as $yfg_svc_value => $yfg_svc_label ) :
										?>
										<label class="yfg-msel__opt">
											<input type="checkbox" name="yfg_service[]" value="<?php echo esc_attr( $yfg_svc_value ); ?>">
											<span><?php echo wp_kses_post( $yfg_svc_label ); ?></span>
										</label>
									<?php endforeach; ?>
								</div>
							</div>
						...
						<script>
						( function () {
							var box = document.getElementById( 'yfgServiceMsel' );
							if ( ! box ) { return; }
							var toggle = box.querySelector( '.yfg-msel__toggle' );
							var label  = box.querySelector( '.yfg-msel__label' );
							var checks = box.querySelectorAll( 'input[type="checkbox"]' );
							var placeholder = label.textContent;

							toggle.addEventListener( 'click', function () {
								box.classList.toggle( 'is-open' );
								toggle.setAttribute( 'aria-expanded', box.classList.contains( 'is-open' ) ? 'true' : 'false' );
							} );
							...
							checks.forEach( function ( c ) {
								c.addEventListener( 'change', function () {
									...
									checks.forEach( function ( x ) {
										if ( x.checked ) { picked.push( x.closest( '.yfg-msel__opt' ).querySelector( 'span' ).textContent.trim() ); }
									} );
									if ( picked.length ) {
