...
// Note: Legacy file with pre-existing PHPCS issues. Direct DB queries required for feed locator.
...
class Feed_Locator{
	...
	public static function should_do_locating()
	{
		$sbr_statuses_option = get_option('sbr_statuses', array());
		if (
			isset($sbr_statuses_option['feed_locator']['initialized'])
	...
	public static function feed_locator_query($args)
	{
		...
		// Build feed_id filter - if specific feed_id provided, filter by it
		$feed_id_filter = "feed_id NOT LIKE '*%'";
		if (isset($args['feed_id']) && ! empty($args['feed_id'])) {
			$feed_id_filter = $wpdb->prepare('feed_id = %s', $args['feed_id']);
		}
